...
dgeom(x, prob)
```
to get the pmfs $P(X=x)$ of random variables following respectively binomial, Poisson, and geometric distributions.

* The first argument is a value $x$ or a vector of values $(x_1, x_2,\ldots, x_n)$.
* The rest arguments are the required parameters for the specification of the distribution.
* It returns a probability $P(X=x)$ or a vector of probabilities $(P(X=x_1), P(X=x_2),\ldots, P(X=x_k))$.

### Cumulative Distriution Functions (CDF)
We can use
```{r}
#| eval: false
pbinom(q, size, prob)
...
pgeom(q, prob)
```
to get the cdfs $P(X\leq q)$ of random variables following respectively binomial, Poisson, and geometric distributions.

* The first argument is a value $q$ or a vector of values $(q_1, q_2,\ldots, q_n)$.
* The rest arguments are the required parameters for the specification of the distribution.
* It returns a probability $P(X\leq q)$ or a vector of probabilities $(P(X\leq q_1), P(X\leq q_2),\ldots, P(X\leq q_k))$.
...
data <- data.frame(
  ...
  pi = unlist(lapply(pi_values, function(pi) dgeom(x, prob = pi))),
  prob = rep(pi_values, each = length(x))
)

# Plot using ggplot2
ggplot(data, aes(x = x, y = pi, color = factor(prob), group = prob)) +
...
data <- data.frame(
  x = rep(x, times = length(pi_values)),
  p = unlist(lapply(pi_values, function(p) dbinom(x, size = n, prob = p))),
  prob = rep(pi_values, each = length(x))
)
...
ggplot(data, aes(x = x, y = p, color = n, group = n)) +
  ...
  labs(
    title = paste0("PMF of Binomial Distribution, fixed ", "π
=0.5"),
    x = "Number of Successes (x)",
    y = "Probability"
  ) +
